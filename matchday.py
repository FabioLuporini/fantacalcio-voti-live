import os
import json
import sys

from utils import (decode_protobuf_live_msg, get_protobuf_message_b64, get_voti,
                   get_signed_uri)
from squadre import codici

# === Helper di normalizzazione ===

def _norm(s: str) -> str:
    return (s or "").strip()


encoded = ""

# Event ID -> str repr
event_mapper = {
   1: "ammonizione",
   2: "espulsione",
   3: "goal",
   4: "goal subito",
   7: "rigore parato",
   8: "rigore sbagliato",
   9: "rigore segnato",  # ?
   10: "autogol",
   11: "unknwon",  # maybe "goal decisivo" ?
   12: "unknwon",  # ?
   14: "sub out",
   15: "sub in",
   16: "goal annullato",
   17: "unknown",  # ?
   20: "unknwon",  # ?
   21: "assist soft",  # ?
   22: "assist",
   23: "unknown",  # ?
   24: "unknown",  # ?
   25: "unknown",  # ?
   26: "unknown",  # ?
   # Custom events
   99: "cleansheet",
}

# Apparently 55 is their designated magic number for SV
SV = 55

modmodulo = {}
modmodulo['P' + 4*'D' + 4*'C' + 2*'A'] = 0
modmodulo['P' + 3*'D' + 5*'C' + 2*'A'] = -0.5
modmodulo['P' + 4*'D' + 3*'C' + 3*'A'] = -1.0
modmodulo['P' + 3*'D' + 4*'C' + 3*'A'] = -1.5
modmodulo['P' + 5*'D' + 3*'C' + 2*'A'] = 0.5
modmodulo['P' + 4*'D' + 5*'C' + 1*'A'] = 1
modmodulo['P' + 5*'D' + 4*'C' + 1*'A'] = 1.5

strmodulo = {}
strmodulo['P' + 4*'D' + 4*'C' + 2*'A'] = '4-4-2'
strmodulo['P' + 3*'D' + 5*'C' + 2*'A'] = '3-5-2'
strmodulo['P' + 4*'D' + 3*'C' + 3*'A'] = '4-3-3'
strmodulo['P' + 3*'D' + 4*'C' + 3*'A'] = '3-4-3'
strmodulo['P' + 5*'D' + 3*'C' + 2*'A'] = '5-3-2'
strmodulo['P' + 4*'D' + 5*'C' + 1*'A'] = '4-5-1'
strmodulo['P' + 5*'D' + 4*'C' + 1*'A'] = '5-4-1'


script_directory = os.path.dirname(os.path.abspath(__file__))


def get_punteggi_lega():
    path = os.path.join(script_directory, 'data', 'punteggi.json')
    with open(path, 'r', encoding='utf-8') as file:
        punteggi = json.load(file)

    punteggi['unknown'] = 0

    return punteggi


def get_ruoli_lega():
    path = os.path.join(script_directory, 'data', 'ruoli.txt')
    with open(path, 'r', encoding='utf-8') as file:
        ruoli = file.readlines()

    mapper = {}
    for i in ruoli:
        # Expected format: ruolo, tabular, name, \n
        pos, name = i.replace("\n", "").split("\t")
        mapper[_norm(name)] = pos

    return mapper


def get_squadre_serieA():
    path = os.path.join(script_directory, 'data', 'serieA.txt')
    with open(path, 'r', encoding='utf-8') as file:
        serieA = file.readlines()

    mapper = {}
    for i in serieA:
        # Expected format: name, tabular, squadra (first three letters), \n
        name, squadra = i.replace("\n", "").split("\t")
        mapper[_norm(name)] = squadra

    return mapper


def parse_fantasquadre():
    path = os.path.join(script_directory, 'data', 'formazioni')
    fantasquadre = []
    for filename in os.listdir(path):
        if filename.startswith('.'):
            continue

        file_path = os.path.join(path, filename)

        if os.path.isfile(file_path):
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                fantasquadre.append(content)

    mapper = {}
    for i in fantasquadre:
        items = i.split("\n")

        # Sanitize (some hand-crafted files might include \t too)
        items = [j.replace("\t", "").strip() for j in items if j.strip()]

        name = items[0]
        manager = items[1] if len(items) > 1 else ""
        # giocatore -> voto; filtra vuoti
        titolari = {p: 0 for p in items[3:14] if _norm(p)}
        panchinari = {p: 0 for p in items[15:-1] if _norm(p)}

        mapper[name] = (titolari, panchinari)

    return mapper


def get_live_data(season_id):
    path = os.path.join(script_directory, 'data', 'giornata.txt')
    with open(path, 'r', encoding='utf-8') as file:
        giornata = int(file.read())

    # Safe: se fallisce la firma, torna struttura vuota
    try:
        signed_uri = get_signed_uri(giornata, season_id=season_id)
        encoded = get_protobuf_message_b64(signed_uri)
        data_live = decode_protobuf_live_msg(encoded[2:-1])
    except KeyError:
        return {"protoData": []}

    return data_live


def inject_custom_events(data):
    matches = data.get('protoData', [])

    # Cleansheet event
    for match in matches:
        goalHome = match.get('goalHome', 0)
        goalAway = match.get('goalAway', 0)

        if goalHome == 0:
            for i in match.get('playersAway', []):
                if i.get('position') == 'P':
                    i.setdefault('events', []).append(99)
                    i.setdefault('eventsMinutes', []).append(120)  # Dummy min
        if goalAway == 0:
            for i in match.get('playersHome', []):
                if i.get('position') == 'P':
                    i.setdefault('events', []).append(99)
                    i.setdefault('eventsMinutes', []).append(120)  # Dummy min


def calc_voto_live(giocatore, punteggi):
    vote = giocatore['vote']
    events = giocatore.get('events', [])

    for event in events:
        event_name = event_mapper.get(event, "unknown")
        bonus_malus = punteggi.get(event_name)
        if bonus_malus is None:
            # E.g., sub out non definito in punteggi
            continue

        # Regola: SV+bonus_malus => 6+bonus_malus
        if vote == SV:
            vote = 6

        vote += bonus_malus

    if vote == 55:
        return 0
    else:
        return vote


def calc_fantasquadra(titolari, panchinari, ruoli):
    # Copia panchina per non mutare input
    panchinari = dict(panchinari)

    tot = 0
    modulo = ""
    unsubbed = []
    for name, voto in titolari.items():
        pos = ruoli.get(_norm(name))

        if voto == 0:
            # Sostituzione stesso ruolo
            for i in list(panchinari):
                subpos = ruoli.get(_norm(i))
                if subpos == pos:
                    voto = panchinari.pop(i)
                    if voto != 0:
                        break

        if voto == 0:
            unsubbed.append(name)

        tot += voto
        modulo += (pos or '')

    # Modulo dinamico
    for name in unsubbed:
        pos = ruoli.get(_norm(name))
        if pos == 'P' or not pos:
            continue

        queue = list(panchinari)
        while queue:
            i = queue.pop(0)

            subpos = ruoli.get(_norm(i))
            voto = panchinari[i]

            if subpos == 'P' or voto == 0:
                continue

            attempted_modulo = list(modulo)
            try:
                attempted_modulo.remove(pos)
            except ValueError:
                continue
            # inserisci dove presente subpos (euristica semplice)
            try:
                index = attempted_modulo.index(subpos)
            except ValueError:
                index = len(attempted_modulo)
            attempted_modulo.insert(index, subpos)
            attempted_modulo = ''.join(attempted_modulo)

            # Se è una formazione legale, applica
            if attempted_modulo in modmodulo:
                modulo = attempted_modulo
                tot += voto
                break

    # Modificatore squadra + stringa modulo
    modulo_bonus = modmodulo.get(modulo, 0)
    tot += modulo_bonus
    return tot, strmodulo.get(modulo, "n/d")


def purge():
    # Move tip to next matchday
    path = os.path.join(script_directory, 'data', 'giornata.txt')
    with open(path, 'r', encoding='utf-8') as file:
        giornata = int(file.read())
    with open(path, 'w', encoding='utf-8') as file:
        file.write("%s" % (giornata + 1))


# === Dump voti su TXT ===

def dump_player_votes(fantasquadre, ruoli, output_path, output_totali=None):
    """
    Scrive un TXT con i voti di ogni giocatore per squadra (titolari e panchina).
    Se output_totali è passato (dict team -> (tot, modulo)), lo aggiunge come intestazione.
    """
    def fmt(v):
        try:
            return f"{float(v):.1f}"
        except Exception:
            return str(v)

    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    with open(output_path, "w", encoding="utf-8") as f:
        for team, (titolari, panchinari) in fantasquadre.items():
            f.write("=" * 60 + "\n")
            f.write(f"{team}\n")
            if output_totali and team in output_totali:
                tot, modulo = output_totali[team]
                f.write(f"Totale: {tot:.1f}   Modulo: {modulo}\n")
            f.write("-" * 60 + "\n")
            f.write("[Titolari]\n")
            for name in sorted(titolari, key=lambda n: (ruoli.get(_norm(n), "Z"), n)):
                ruolo = ruoli.get(_norm(name), "?")
                f.write(f"{ruolo}\t{name}\t{fmt(titolari[name])}\n")
            f.write("\n[Panchina]\n")
            for name in sorted(panchinari, key=lambda n: (ruoli.get(_norm(n), "Z"), n)):
                ruolo = ruoli.get(_norm(name), "?")
                f.write(f"{ruolo}\t{name}\t{fmt(panchinari[name])}\n")
            f.write("\n")


if __name__ == "__main__":
    # Launch: python matchday.py
    # Prerequisites:
    # * `punteggi.json` must be available
    # * `ruoli.txt` must be available
    # * `serieA.txt` must be available
    # * `formazioni/` must be available

    punteggi = get_punteggi_lega()
    ruoli = get_ruoli_lega()

    squadre = get_squadre_serieA()

    fantasquadre = parse_fantasquadre()

    if len(sys.argv) == 1:
        # Testing!
        data = decode_protobuf_live_msg(encoded)
    else:
        assert len(sys.argv) >= 2
        if sys.argv[1] == 'live':
            season_id = 20
            data = get_live_data(season_id)
        elif sys.argv[1] == 'next':
            purge()
            sys.exit(0)
        else:
            raise ValueError

    inject_custom_events(data)

    unplayed = []
    for k, v in codici.items():
        serie_a_team = get_voti(data, v)

        if not serie_a_team:
            unplayed.append(k)
            continue

        for giocatore in serie_a_team:
            for team, (titolari, panchinari) in fantasquadre.items():
                name = giocatore['name']
                if name in titolari:
                    titolari[name] = calc_voto_live(giocatore, punteggi)
                elif name in panchinari:
                    panchinari[name] = calc_voto_live(giocatore, punteggi)
                else:
                    continue

    # Amend vote if player hasn't played yet (S.V. 6)
    for titolari_panchinari in fantasquadre.values():
        for m in titolari_panchinari:
            for name, vote in list(m.items()):
                n = _norm(name)
                if not n:
                    m.pop(name, None)
                    continue
                abbr = squadre.get(n)
                if abbr and any(i.startswith(abbr) for i in unplayed):
                    m[name] = 6  # S.V.

    output = {team: calc_fantasquadra(titolari, panchinari, ruoli)
              for team, (titolari, panchinari) in fantasquadre.items()}

    totali = {k: v for k, (v, _) in output.items()}
    table = sorted(totali, key=lambda i: output[i][0], reverse=True)

    print([(i, *output[i]) for i in table])

    # Salva sempre un file TXT con i dettagli dei voti
    dump_path = os.path.join(script_directory, "data", "voti_giocatori.txt")
    dump_player_votes(fantasquadre, ruoli, dump_path, output_totali=output)
    print(f"\n[INFO] File voti salvato in: {dump_path}")
